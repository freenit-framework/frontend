<script lang="ts">
  ...
  let { courseId, groupId, store } = $props()
  let loading = $state(true)

  onMount(async () => {
    const [groupResponse, membersResponse, usersResponse] = await Promise.all([
      store.lms.fetchGroup(groupId),
      store.lms.fetchGroupMembers(groupId),
      store.user.fetchAll(),
    ])
    if (!groupResponse.ok) {
  ...
  async function fetchPrevious() {
    const response = await store.lms.fetchGroupMembers(groupId, store.user.list.page - 1)
    if (!response.ok) {
  ...
  function isMember(userId: number) {
    return store.lms.groupMembers.some((member: any) => member.user_id === userId)
  }
  ...
  async function toggleMember(user: any, event: Event) {
    ...
    let response
    if (target.checked) {
      response = await store.lms.addGroupMember(groupId, user.id)
    } else {
      response = await store.lms.removeGroupMember(groupId, user.id)
    }
    if (!response.ok) {
      notification.error(response.statusText)
...
{:else}
  <div class="container">
    <div class="header">
      <div>
        <h2>{store.lms.groupDetail.name}</h2>
        <p>{store.lms.groupDetail.description || ''}</p>
      </div>
      ...
      {#each store.user.list.data as user (user.id)}
        ...
        <div class="data">
          <input
            ...
            onchange={(e) => toggleMember(user, e)}
          />
        </div>
